# Plan: env-iup release v0.3 (atmos v0.8)
...
3. [x] Phase 1 tests (local LUA_PATH): hello exit-0;
       button-counter + iup-net run clean (6s smoke;
       iup-net used installed env-socket rock, code == v0.3)
4. [x] Rockspec `atmos-env-iup-0.3-1` (branch=v0.3,
       dep `atmos ~> 0.8`); dev-1 unchanged (unversioned dep)
5. [ ] Make rockspec (sudo -- user):
       `sudo luarocks --lua-version=5.4 make atmos-env-iup-0.3-1.rockspec`
6. [ ] Phase 2 tests (global): hello / button-counter / iup-net
